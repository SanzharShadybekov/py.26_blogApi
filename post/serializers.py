...
class PostDetailSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        repr['comments'] = CommentSerializer(
            instance=instance.comments.all(), many=True
        ).data  # 2ой способ
        repr['likes_count'] = instance.likes.count()
        repr['liked_users'] = LikedUsersSerializer(
            instance=instance.likes.all(), many=True).data
        user = self.context['request'].user
        if user.is_authenticated:
            repr['is_liked'] = self.is_liked(instance, user)
            repr['is_favorite'] = self.is_favorite(instance, user)
...
class PostCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for image in images_data:
            PostImages.objects.create(image=image,
                                      post=post)
        return post
